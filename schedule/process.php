...
switch($action){
	...
	case "get_categories"; //calendars
		...
		if($categories !=0 ){
			foreach($categories as $key=>$category){
				...
				$result = mysql_query($sql);
				$response .= '<div style="display:none;" id="category_'.$category[id].'">';
	...
	case "get_modal_content":
		...
		switch($modal_type){
			case "add_contact":
				$account_info = $accounts_obj->get_user_by_id($_SESSION['user']['logged_in_id']);

				if($account_info['subscription'] == "free"){
					$num_contacts = $contacts_obj->get_user_contacts_count($_SESSION['user']['logged_in_id']);
					if($num_contacts == 10){
						$data['feature'] = 'Contacts (10)';
			...
			case "edit_category":
				...
				$response = $data_template;			
				break;
			case "manage_categories": //revised version as of 09/24/2011
				...
				if($categories != 0){
					...
					foreach($categories as $category){
						if(count($selected_categories) > 0){
							if(in_array($category[id],$selected_categories)){
								$user_categories .= '<tr><td width="100">'.$category['name'].'</td><td><a href="###" class="edit_category">edit</a></td><td width="50"><a href="###" class="delete_category" id="'.$category['id'].'">delete</a></td></tr>';
							}else{
								$user_categories .= '<tr><td width="100">'.$category['name'].'</td><td><a href="###" class="edit_category">edit</a></td><td width="50"><a href="###" class="delete_category" id="'.$category['id'].'">delete</a></td></tr>';
							}
						}else{
							$user_categories .= '<tr><td width="100">'.$category['name'].'</td><td width="50"><a href="###" class="edit_category">edit</a></td><td><a href="###" class="delete_category" id="'.$category['id'].'">delete</a></td></tr>';
						}						
					}
					$user_categories .= '</table>';
				}else{
				...
				print $content;
				break;				
			case "manage_categories_v1": //original implementation
				$modal_content_template = file_get_contents('../templates/manage_categories.tpl');
				$categories = $categories_obj->get_user_categories($_SESSION['user']['logged_in_id']);
				if($categories != 0){
					$user_categories .= '<table>';
					foreach($categories as $category){
						if(count($selected_categories) > 0){
							if(in_array($category[id],$selected_categories)){
								$user_categories .= '<tr><td width="100"><input name="category[]" type="checkbox" id="'.$category[name].'" value="'.$category[id].'"><a href="###" onclick="show_modal(\''.edit_category.'\','.$category[id].')">'.$category['name'].'</a></td><td><a href="###" class="delete_category">delete</a></td></tr>';
							}else{
								$user_categories .= '<tr><td width="100"><input name="category[]" type="checkbox" id="'.$category[name].'" value="'.$category[id].'"><a hr="###" onclick="show_modal(\''.edit_category.'\','.$category[id].')">'.$category['name'].'</a></td><td><a href="###" class="delete_category">delete</a></td></tr>';
							}
						}else{
							$user_categories .= '<tr><td width="100"><input name="category[]" type="checkbox" id="'.$category[name].'" value="'.$category[id].'"><a href="###" onclick="show_modal(\''.edit_category.'\','.$category[id].')">'.$category['name'].'</a></td><td><a href="###" class="delete_category">delete</a></td></tr>';
						}						
			...
			case "bulk_add":
				...
				if($account_info['subscription'] == "free"){
					...
					if($num_contacts == 10){
						$data['feature'] = 'Contacts (10)';
						$template = file_get_contents('../templates/max_features.tpl');
						$modal_content_template = replace_var($data,$template);
					}else{
	...
	case "edit_category":
		...
		if(count($_POST['manager_ids']) > 0){ //If the category has Calendar Managers assigned check if the given Manager has an account or not
				...
				if($manager != ""){				
					...
					if($accounts_obj->check_account_exists($contact['email'])){
						...
						$mail_params['from_email'] = $_SESSION['user']['email'];
						$mail_params['from_name'] = $_SESSION['user']['first_name'];
						$mail_params['to_email'] = $contact['email'];
						$mail_params['subject'] = "You are now a manager";
						$template = file_get_contents('../templates/calendar_manager_notification.tpl');
						//$data['manager_name'] = $contact['first_name'] . ' ' .$contact['last_name'];
						$mail_params['message_body'] = replace_var($contact,$template);						
						//$mail_params['message_body'] = "Please manage the calendar";
						mail_out($mail_params); //utilities.php
					}
	...
	case "add_event":
		...
		//$_POST['user'] = $_SESSION['user']['logged_in_id']; //commented out on 03/09/2011
	...
	case "delete_associated_events":
		$result = $events_obj->delete_events_by_calendar_id($_POST['category_id']);
		if($result == 1){
			$del_calendar_result = $categories_obj->delete_category_by_id($_POST['category_id']); //Delete the Calendar
