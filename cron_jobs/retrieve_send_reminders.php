...
$begin_marker = $cdt - 300; //begin marker is 5 mins before current time
$end_marker = $cdt + 300; //end marker is 5 mins after current time
$sql = "SELECT * FROM `event_reminders` WHERE `reminder_send_time` >= '$begin_marker' AND `reminder_send_time` <= '$end_marker' AND `reminder_notification`='1hourbefore'";
//echo $sql;
...
if($result){
	if(mysql_num_rows($result) > 0){
		...
		$events = $reminders_obj->get_events_by_list($event_ids);
		if($events != 0){
			foreach($events as $event){
				...
				send_reminders($event,$contacts); //send reminders for current event to list of contacts
			}
		}
	}
}
...
function send_reminders($event,$contacts){
