...
extract($_POST);
$contacts = explode(",",$email_list); //separate contacts
$num_user_submitted_contacts = count($contacts);
...
if($allowance == 0){
	...
	if($num_user_submitted_contacts < $allowance){
		...
		foreach($contacts as $contact){ //Loop through contacts and for each contact separate the email address from the phone number
			$contact_info = explode(":",$contact);
			$contact_email = $contact_info[0];
			$contact_phone = $contact_info[1];
			$query = "INSERT INTO () VALUES ('$contact_email','$contact_phone')";
			print '<pre>';
			print $query;
			print '</pre>';
		}
