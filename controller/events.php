...
$args['end'] = $_GET['end']; 
$events = $events_obj->get_user_events_date_range($args); //Retrieve events associated with Calendars that the currently logged in user owns
...
//print_r($other_calendars);
if($other_calendars != 0){
	foreach($other_calendars as $calendar){ //Iterate through Other Calendars and take each calendar id and place in local array
...
$event_data = "";
if($events != 0 || $events_user_manages != 0){
	if($events != 0){
